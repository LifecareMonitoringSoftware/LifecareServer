...
var (
	...
	nameProperRegExp             = regexp.MustCompile(`[^a-zA-Z0-9-',.]`)                                                         // Characters recognized in surnames and proper names
...
func Email(original string, preserveCase bool) string {
	...
	if preserveCase {
		result = string(emailRegExp.ReplaceAll(
...
// NameProperSingle returns a sanitized name (for individual First, Middle, and Last names).
func NameProperSingle(original string) string {
	var result = string(nameProperRegExp.ReplaceAll([]byte(original), emptySpace))
	return result
}
...
// Phone,
// This strips all non digit characters from a phone number.
// Any string may be supplied. The returned string will only contain digits.
func Phone(original string) string {
	var result strings.Builder
	for i := 0; i < len(original); i++ {
		byteChar := original[i]
		if '0' <= byteChar && byteChar <= '9' {
			result.WriteByte(byteChar)
		}
	}
